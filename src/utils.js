    ...
    .map(([ch, d]) => {
      ...
      rows.forEach(r => {
        if (ch === 'meta') {
          ...
          s += `- "${r.name}": Spend $${r.spend.toFixed(0)}, Purchases ${r.purchases}, ROAS ${r.roas.toFixed(2)}x, CPA $${cpa}, Frequency ${r.frequency.toFixed(2)}, CPM $${r.cpm.toFixed(2)}, Impressions ${r.impressions.toLocaleString()}, Reach ${r.reach.toLocaleString()}\n`;
        } else if (ch === 'google') {
          s += `- "${r.name}": Spend $${r.spend.toFixed(0)}, Conversions ${r.conversions.toFixed(0)}, ROAS ${r.roas.toFixed(2)}x, CPA $${r.cpa.toFixed(2)}, CTR ${r.ctr.toFixed(2)}%, Impressions ${r.impressions.toLocaleString()}\n`;
        } else {
