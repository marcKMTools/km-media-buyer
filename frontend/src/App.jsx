...
function Badge({ type, children }) {
  const colors = { g:[C.teal,'rgba(0,224,176,0.12)'], w:[C.amber,'rgba(255,178,36,0.12)'], r:[C.coral,'rgba(255,107,91,0.12)'], v:[C.violet,'rgba(139,111,255,0.12)'] }
...
export default function App() {
  ...
  const processFile=useCallback((file,channel)=>{
    ...
    reader.onload=e=>{
      ...
      if(!rows.length){alert('No campaigns with spend found');return}
  ...
  return (
    <div style={{background:'#080809',minHeight:'100vh',color:'#e8e8ee',fontFamily:"'Epilogue', sans-serif"}}>
      ...
      <div style={{padding:'20px 24px',maxWidth:1200}}>
        {error&&<div style={{background:'rgba(255,107,91,0.1)',border:'1px solid rgba(255,107,91,0.3)',borderRadius:8,padding:'12px 16px',marginBottom:14,...mono,fontSize:11,color:C.coral}}>{error}</div>}
          ...
          <div>
            ...
            {!hasData?(
              <div style={{textAlign:'center',padding:'52px 20px'}}>
                ...
                <div style={{...mono,fontSize:11,color:'#666',lineHeight:1.8,marginBottom:20}}>Meta Ads Manager · Google Ads — auto-detected from headers</div>
                <UploadZone channel="auto" onFile={processFile} label="Drop any ad CSV here" hint="Meta or Google Ads campaign export"/>
              </div>
            ):(
              <div>
                <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:10,marginBottom:14}}>
              ...
              <div>
                ...
                <Card>
                  ...
                  <div style={{display:'flex',gap:10,alignItems:'flex-end'}}>
                    <textarea value={chatInput} onChange={e=>setChatInput(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();sendChat()}}} placeholder="e.g. Should I scale Product Sets ASC this week?" style={{flex:1,background:'#161619',border:'1px solid rgba(255,255,255,0.12)',borderRadius:8,padding:'10px 14px',color:'#e8e8ee',...mono,fontSize:12,resize:'none',minHeight:42,maxHeight:100,outline:'none'}}/>
